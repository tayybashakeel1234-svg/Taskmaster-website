...
const resolvers = {
  ...
  Mutation: {
    ...
    // 🔥 Updated updateTask mutation with safe date parsing
    updateTask: async (_, { id, title, description, status, due_date }) => {
      let parsedDate = null
      if (due_date) {
        // Agar valid date string hai to PostgreSQL friendly format me convert kar do
      ...
      const result = await pool.query(
        `UPDATE tasks
         ...
         WHERE id=$5
         RETURNING *`,
        [title, description, status, parsedDate, id]
      )
      return result.rows[0]
    },
